...
//    var item: Item? {
//        didSet {
//            print("Item was pushed along. Showing Detail on \(String(describing: item?.name))")
...
//        guard let name = nameCell.itemNameTextField.text, name != "" else { return }
//        print(name)
...
//        let warrantyDate = dateFormatter.date(from:"\(itemCell.warrantyExpirationDateLabel.text ?? "")") ?? Date()
//        print(warrantyDate)
//        guard let noteCell = tableView.cellForRow(at: IndexPath(row: 0, section: 3 )) as? NotesCell else { return }
//        let note = noteCell.notesTextView.text ?? ""
//        
//        let photos = CoreDataController.shared.photos
//        // 2. we save item to Core Data
//        
//        CoreDataController.shared.createItem(category: categoypicked, photos: photos, name: name, isFavorite: favorite, returnDate: returnDate, modelNumber: modelNumber, note: note, price: itemPrice, storePurchasedFrom: vendor, quantity: quantity!, serialNumber: serialNumber, warrantyDate: warrantyDate, purchaseDate: purchaseDate)
//        
//        
...
//    func getPhotoForThumbNail(with item: Item) -> UIImage {
//        
...
//    }
//
//    func getAllPhotosForItem(with item: Item) -> [UIImage] {
//        
//    guard let photos = item.images?.array as? [Image] else {return [UIImage()]}
//        
